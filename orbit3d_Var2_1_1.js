...
const DEBUG_PRESETS = {
    stableOrbit: {
        ...
        ui: {
            ...
            objMass: 1.0,        objMassInput: 1.0, // 🛡️ 文字列の"1.0"を純粋な数値1.0に修正！
            useFixedObjMass: false,
        ...
        physics: () => {
            ...
            if (typeof initialBodyCount !== "undefined") window.initialBodyCount = 80;
                        // ⚠️ 記述スタイルを統一し、確実にグローバル空間の設定をハックする規律
            if (window.settings) {
    ...
    chaosCollapse: {
        ...
        ui: {
            ...
            sunVxSlider: 2.0,    sunVxInput: 2.0,  // 🛡️ スライダー限界を考慮し、安全な2.0に調律
            sunVySlider: -2.0,   sunVyInput: -2.0, // 🛡️ 同上
            sunVzSlider: 0,      sunVzInput: 0,
        ...
        physics: () => {
            ...
            if (window.settings) {
                settings.sunInitialVx = 2.0;       // 🛡️ UI側の値と完全に同期
                settings.sunInitialVy = -2.0;      // 🛡️ 同上
                settings.sunInitialVz = 0.0;
...
function resetCameraMemory() {
  ...
  // メインループが「カメラが動いた！」と誤認して古い座標で描画するのを防ぐ
...
function updatePhysics(dt) {
 ...
 // =======================================================
  ...
  // =======================================================
  if (!fullGravity) {
    // 🛡️ エラー防止のための安全弁：もしUI側に変数がない場合でもバグらない規律
    ...
    // ⚠️ 【核心の規律】太陽（i=0）も含めて、お互いが受ける力を計算するために「1本のループ」で全天体をフラットに処理！
...
function renderScene() {
  ...
  for (const obj of sortedBodies) {
    const pr = project3D(obj.r.x, obj.r.y, obj.r.z);
        // ニア・クリッピングガード：カメラの真後ろにいる天体は、描画計算そのものをスキップ
    if (!pr.visible) continue;

    // 安全版カラーの取得
    const trailColor = getTrailColor(obj.b, sun);
        // 動的なスクリーンサイズの計算（遠近感の厳密適用）
    const screenSize = calculateScreenSize(obj.b, pr);

    // 各描画フェーズを、完全に独立した専門関数へ委託するわよ！
  ...
  if (simulationState.ui.showNames) {
    for (const obj of sortedBodies) {
      const pr = project3D(obj.r.x, obj.r.y, obj.r.z);
      if (!pr.visible) continue; // 画面外・カメラ後方は当然スキップ
...
function drawBodyTrails(b, trailColor) {
  ...
  if (cameraChanged || !b.trail[0] || b.trail[0].sx === undefined){
    for (let i = 0; i < totalPoints; i++) {
      const p = b.trail[i]; // pは物理世界の絶対座標 {wx, wy, wz} （またはx, y, z）
            // 【ここを乗っ取る！】カメラに渡す直前でSunの位置を引き算して「相対化」
      // あなたの元のコードの p.wx, p.wy, p.wz から、center の座標を引くのよ
      const relX = p.wx - center.x;
      const relY = p.wy - center.y;
      const relZ = p.wz - center.z;

      // 差し引いた相対座標を、既存の rotate3D / project3D に流し込む！
      const rp = rotate3D({ x: relX, y: relY, z: relZ });
      const prp = project3D(rp.x, rp.y, rp.z);
            p.sx = prp.x;           // 最新の2D座標に安全に上書き
      p.sy = prp.y;
  ...
  ctx.save();
    // 線の基本太さを事前定義（ループの外側で1回だけ計算する規律ね）
...
for (let i = 0; i < totalPoints - 1; i++) {
  ...
  // ⚠️ 太陽であっても、カメラの真後ろ（見えない領域）にある座標同士は結ばない！
  // これにより、カメラを背けた時の「画面を切り裂く謎の直線バグ」が完全に消滅するわ。
    ...
    function executeButtonCoreLogic(id, element) {
        switch (id) {
            case "btnToggleRotate":
                ...
                // ボタンの表示文字を同期
                element.textContent = window.isAutoRotateEnabled ? "自動回転: ON" : "自動回転: OFF";
                break;
